...
class TradeMonitor:
    ...
    def __init__(self, config: Dict, magic_number: int = 123456):
        ...
        logger.info(f"TradeMonitor initialized - Trailing: {self.config.trailing_enabled}, "
    ...
    def _apply_partial_close(self, position, current_rr: float) -> Optional[Dict]:
        ...
        if result.success:
            logger.info(f"💰 PARTIAL CLOSE: {symbol} #{position.ticket} - {close_pct:.0f}% fermé à RR {current_rr:.2f}")
            return {'action': 'partial_close', 'ticket': position.ticket, 'symbol': symbol, 'rr': current_rr}
        return None
    ...
    def _apply_break_even(self, position, current_rr: float) -> Optional[Dict]:
        ...
        if success:
            logger.info(f"🔒 DYNAMIC BE: {symbol} #{position.ticket} activé à RR {current_rr:.2f} (SL: {new_sl})")
            return {'action': 'break_even', 'ticket': position.ticket, 'symbol': symbol, 'rr': current_rr}
        return None
    ...
    def _check_trailing_stop(self, position, profit_pips: float, state: Dict) -> Optional[Dict]:
        ...
        activation_pips = symbol_overrides.get('trailing_start_pips', self.config.trailing_activation_pips)
        distance_pips = symbol_overrides.get('trailing_distance_pips', self.config.trailing_distance_pips)
        ...
        if position.type == mt5.ORDER_TYPE_BUY:
            ...
            if position.sl < optimal_sl - (pip_size * 0.5):
                ...
                if success:
                    logger.info(f"📈 TRAILING: {symbol} #{position.ticket} - SL trailing à {optimal_sl:.5f}")
                    return {
