...
class MomentumConfirmationFilter:
    ...
    def check_sell_confirmation(
        ...
        logger.info(f"   🔍 Zone Premium Extrême ({premium_percent:.1f}%). Vérification confirmation...")
        ...
        if vol_col in df.columns:
             ...
             if avg_vol > 0:
                 ...
                 if rvol < 0.7:
                     logger.warning(f"   ❌ SELL BLOQUÉ : Volume trop faible (RVOL: {rvol:.2f} < 0.7) - Faux mouvement probable")
        ...
        if is_bearish and has_rejection_wick:
            logger.info(f"   ✅ Confirmation : Bougie de Rejet détectée (Wick: {upper_wick:.1f} vs Body: {body:.1f})")
        ...
        if avg_range < (atr_value / 2):
            logger.info(f"   ✅ Confirmation : Pause du momentum (Avg Range: {avg_range:.1f} < ATR/2: {atr_value/2:.1f})")
        ...
        logger.warning(f"   ❌ SELL BLOQUÉ : Zone Premium Extrême ({premium_percent:.1f}%) sans confirmation de rejet")
    ...
    def check_buy_confirmation(
        ...
        logger.info(f"   🔍 Zone Discount Extrême ({premium_percent:.1f}%). Vérification confirmation...")
        ...
        if vol_col in df.columns:
             ...
             if avg_vol > 0:
                 rvol = current_vol / avg_vol
                 if rvol < 0.7:
                     logger.warning(f"   ❌ BUY BLOQUÉ : Volume trop faible (RVOL: {rvol:.2f} < 0.7) - Faux mouvement probable")
                     return False, f"❌ Low Volume (RVOL: {rvol:.2f})"
        ...
        if is_bullish and has_bounce_wick:
            logger.info(f"   ✅ Confirmation : Bougie de Rebond détectée (Wick: {lower_wick:.1f} vs Body: {body:.1f})")
        ...
        logger.warning(f"   ❌ BUY BLOQUÉ : Zone Discount Extrême ({premium_percent:.1f}%) sans confirmation de rebond")
