...
class RiskManager:
    ...
    def calculate_position_size(self, account_balance: float, entry_price: float,
        ...
        pip_value = symbol_info.get('pip_size', self._get_pip_value(symbol)) if symbol_info else self._get_pip_value(symbol)
        pip_val_per_lot = symbol_info.get('pip_value_per_lot', self._get_pip_value_per_lot(symbol)) if symbol_info else self._get_pip_value_per_lot(symbol)
        
        sl_pips = abs(entry_price - stop_loss) / pip_value if pip_value > 0 else 10
        if sl_pips == 0: sl_pips = 1
        
        lot_size = risk_amount / (sl_pips * pip_val_per_lot) if (sl_pips * pip_val_per_lot) > 0 else 0.01
        ...
        if account_balance < 1000:
            ...
            if lot_size >= SMALL_ACCOUNT_MAX_LOT:
                logger.warning(f"🛡️ Small account protection: Lot capped to {SMALL_ACCOUNT_MAX_LOT} (capital: ${account_balance:.2f})")
        ...
        if lot_size > ABSOLUTE_MAX_LOT:
            logger.error(f"🚨 CRITICAL: Lot size {lot_size} exceeds ABSOLUTE MAX {ABSOLUTE_MAX_LOT}! Bug detected!")
            logger.error(f"🚨 Details: symbol={symbol}, balance=${account_balance}, risk_amount=${risk_amount}, sl_pips={sl_pips}, pip_val_per_lot={pip_val_per_lot}")
            lot_size = ABSOLUTE_MAX_LOT
        
        # Validation finale
        if lot_size <= 0 or lot_size != lot_size:  # NaN check
            logger.error(f"🚨 CRITICAL: Invalid lot_size calculated: {lot_size}. Using minimum 0.01")
            lot_size = 0.01
    ...
    def calculate_trailing_stop(self, entry_price: float, current_price: float,
        ...
        if not self.use_trailing: return None
        pip_val = self._get_pip_value(symbol)
    ...
    def calculate_break_even(self, entry_price: float, current_price: float,
                             current_sl: float, direction: str, symbol: str = "EURUSD") -> Optional[float]:
        if not self.use_break_even: return None
        pip_val = self._get_pip_value(symbol)
        trigger = self.be_pips * pip_val
        if direction.upper() == "BUY":
            if current_price - entry_price >= trigger:
                if current_sl < entry_price: return entry_price
        else:
    ...
    def calculate_position_size_dynamic(self, account_balance: float, entry_price: float,
                                        stop_loss: float, symbol: str, mt5_api: Any) -> PositionSize:
        """Calcule la taille de position en utilisant les données dynamiques du broker via MT5."""
        symbol_info = mt5_api.get_symbol_info(symbol) if hasattr(mt5_api, 'get_symbol_info') else None
        return self.calculate_position_size(account_balance, entry_price, stop_loss, symbol, symbol_info)
    ...
    def can_open_trade(self, symbol: str) -> Tuple[bool, List[str]]:
        ...
        if self.daily_pnl <= -self.max_daily_loss:
            reasons.append(f"Daily loss limit reached ({self.daily_pnl:.2f}% <= -{self.max_daily_loss}%)")
        ...
        if symbol_trades >= self.max_trades_per_symbol:
            reasons.append(f"Max trades for {symbol} reached ({symbol_trades}/{self.max_trades_per_symbol})")
            
        # 4. Vérifier la limite de trades quotidiens
        if self.daily_trades >= self.max_daily_trades:
            reasons.append(f"Max daily trades reached ({self.daily_trades}/{self.max_daily_trades})")
        ...
        if symbol in self.loss_cooldowns:
            if now < self.loss_cooldowns[symbol]:
                ...
                reasons.append(f"Cooldown active for {symbol} after loss ({remaining:.1f} min remaining)")
        ...
        if self.consecutive_losses.get(symbol, 0) >= self.max_consecutive_losses:
            reasons.append(f"Max consecutive losses reached for {symbol} ({self.consecutive_losses[symbol]})")
    ...
    def on_trade_closed(self, pnl_percent: float, symbol: str):
        ...
        if pnl_percent < 0:
            ...
            self.loss_cooldowns[symbol] = datetime.now() + timedelta(minutes=self.cooldown_after_loss)
            logger.warning(f"📉 Perte sur {symbol} ({pnl_percent:.2f}%). Cooldown: {self.cooldown_after_loss}min. Pertes consécutives: {self.consecutive_losses[symbol]}")
        else:
    ...
    def get_anti_tilt_status(self) -> Dict:
        return {
            'equity': {'current': self.current_equity, 'peak': self.peak_equity, 'drawdown': self.current_drawdown_pct},
            'daily_trades': self.daily_trades,
