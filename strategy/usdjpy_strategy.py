...
class UsdJpySMCStrategy:
    ...
    def analyze(self) -> Optional[UsdJpySignal]:
        ...
        if htf_zone != "premium" and htf_zone != "equilibrium":
             ...
             if htf_bias == "bullish":
                 logger.info(f"   ❌ Contexte D1: Tendance Bullish et Zone Discount/Equilibrium -> Pas de short. Zone={htf_zone}")
                 return None

        logger.info(f"   ✅ Contexte D1 favorable pour un Short de retracement/rejet: Tendance={htf_bias}, Zone={htf_zone}")
        ...
        if not bearish_ob:
            ...
            return None
        logger.info(f"   ✅ Order Block Baissier H4 trouvé: {bearish_ob['high']:.3f} - {bearish_ob['low']:.3f}")
    ...
    def _find_h4_bearish_order_block(self) -> Optional[dict]:
        ...
        for i in range(start_index, max(len(df)-20, 0), -1):
            ...
            next_candles_drop = (df['close'].iloc[i] - df['close'].iloc[i+3]) / self.pip_value # Drop sur 3 bougies
