...
class AntiTiltSystem:
    ...
    def update_equity(
        self, current_equity: float, trade_pnl: float = None, is_win: bool = None
    ) -> TradingState:
        """
    ...
    def _record_trade(self, pnl: float, is_win: bool = None):
        ...
        self.trade_history.append({"pnl": pnl, "is_win": is_win, "time": datetime.now()})
        ...
        for streak_count, (action, duration) in self.LOSS_STREAK_ACTIONS.items():
            if losses >= streak_count:
                if action == "PAUSE":
                    ...
                    logger.warning(
                        f"⏸️ PAUSE activée: {losses} pertes consécutives. "
                        f"Trading bloqué jusqu'à {self.state.locked_until.strftime('%H:%M')}"
                    )
                elif action == "STOP":
                    self.state.risk_level = "STOPPED"
                    logger.critical(
                        f"🛑 STOP TRADING: {losses} pertes consécutives! " f"Reset manuel requis."
                    )
    ...
    def _calculate_kelly(self):
        ...
        R = (
            abs(self.state.recent_avg_win / self.state.recent_avg_loss)
            if self.state.recent_avg_loss != 0
            else 0
        )
        ...
        logger.debug(
            f"📊 Kelly: Optimal={self.state.kelly_optimal:.2f}%, "
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "daily": {"trades_today": self.state.trades_today, "max_trades": self.max_daily_trades},
            "recent_performance": {
...
if __name__ == "__main__":
    ...
    system = AntiTiltSystem({"base_risk_percent": 1.0, "use_kelly": True, "kelly_fraction": 0.25})
    ...
    for i, pnl in enumerate(trades):
        ...
        print(
            f"  Base: {risk_result.base_risk_percent:.2f}% → Adjusted: {risk_result.adjusted_risk_percent:.2f}%"
        )
