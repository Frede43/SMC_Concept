...
class NewsFilter:
    ...
    def __init__(self, config: Dict):
        ...
        self.pause_before = news_config.get(
            "minutes_before", news_config.get("pause_minutes_before", 30)
        )
        self.pause_after = news_config.get(
            "minutes_after", news_config.get("pause_minutes_after", 15)
        )
        self.filter_high = news_config.get(
            "filter_high_impact", news_config.get("avoid_high_impact", True)
        )
        ...
        logger.info(
            f"NewsFilter initialized - Enabled: {self.enabled}, "
    ...
    def is_trading_allowed(self, symbol: str) -> Tuple[bool, Optional[str]]:
        ...
        for event in self.events_cache:
            ...
            is_speech = any(
                k in event.event.lower()
                for k in ["speak", "speech", "testif", "conference", "minutes"]
            )
            if is_speech:
    ...
    def check_emergency_exit(
        self, symbol: str, horizon_minutes: int = 5
            ...
            if self.cache_file.exists():
                with open(self.cache_file, "r", encoding="utf-8") as f:
                    ...
                    if (now - cache_time).total_seconds() < duration:
                        ...
                        self.cache_date = now
                        logger.debug(
                            f"📰 Loaded {len(self.events_cache)} events from cache ({self.api_source})"
                        )
                        return
                    ...
                    try:
                        ...
                        if "T" in date_str:
                            ...
                            if "-05:00" in date_str:
                                ...
                                event_datetime = event_datetime + timedelta(
                                    hours=self.timezone_offset + 5
                                )
                            ...
                            event_datetime = datetime.strptime(date_str, "%Y-%m-%d").replace(
                                hour=12
                            )
    ...
    def _merge_and_validate_sources(
        self, sources: List[Tuple[str, List[EconomicEvent]]]
    ) -> List[EconomicEvent]:
        """
        ...
        for source_name, events in sources:
            for event in events:
                ...
                time_rounded = event.time.replace(
                    minute=(event.time.minute // 15) * 15, second=0, microsecond=0
                )
        ...
        for key, event_list in event_groups.items():
            if len(event_list) == 1:
                ...
                best_event = max(event_list, key=lambda x: impact_priority.get(x[1].impact, 0))[1]
                ...
                if best_event.impact == "high":
                    ...
                    logger.debug(
                        f"📊 Cross-validated HIGH event: {best_event.event} "
